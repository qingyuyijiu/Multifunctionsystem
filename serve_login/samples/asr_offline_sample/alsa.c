...
int run_asr(UserData *udata)
{
	...
	asr_audiof = "./wav/hehe.pcm";
	f_pcm = fopen(asr_audiof, "rb");
	if (NULL == f_pcm)
	{
		printf("打开\"%s\"失败！[%s]\n", (char *)f_pcm, strerror(errno));
		goto run_error;
	}
	...
	if (NULL != rec_rslt)
	{
		...
		int id = atoi(ptr);
		printf("语音识别的id: %d\n", id);
		ServerSendbag("sbid", strlen(ptr), ptr); // 获取到的id发回去
	}
	else
	{
		printf("没有识别结果！\n");								 // 给客服端发送一个错误的指令
		ServerSendbag("sbid", strlen("no"), "no"); // 没有识别
	}
	printf("=============================================================\n");
...
void *alsa_init(void *arg)
{
	const char *login_config = "appid = 8279085d"; // 登录参数
	int ret = 0;
	// char c;
